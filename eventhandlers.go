...
// HandleGetSliTriggeredEvent handles get-sli.triggered events if SLIProvider == python-provisioner-service
...
func HandleGetSliTriggeredEvent(myKeptn *keptnv2.Keptn, incomingEvent cloudevents.Event, data *keptnv2.GetSLITriggeredEventData) error {
	...
	if data.GetSLI.SLIProvider != "python-provisioner-service" {
	...
	// Get SLI File from python-provisioner-service subdirectory of the config repo - to add the file use:
	//   keptn add-resource --project=PROJECT --stage=STAGE --service=SERVICE --resource=my-sli-config.yaml  --resourceUri=python-provisioner-service/sli.yaml
	sliFile := "python-provisioner-service/sli.yaml"
